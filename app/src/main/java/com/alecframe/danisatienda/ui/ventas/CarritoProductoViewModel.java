...
public class CarritoProductoViewModel extends AndroidViewModel {
    ...
    public CarritoProductoViewModel(@NonNull Application application) {
        super(application);
    ...
    public void cargarListaProductos(String nombre, List<Producto> productosEnLista) {
        ...
        Call<List<Producto>> call = servicio.obtenerProductos(nombre, idCategoria, 1, null);
        call.enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<List<Producto>> call, Response<List<Producto>> response) {
                if (response.isSuccessful()) {
                    List<Producto> list = new ArrayList<>(response.body());
                    list.removeAll(productosEnLista);

                    mListaProductos.postValue(list);
                } else {
